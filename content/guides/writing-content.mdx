...
### Excluding files with `_`

Files and directories whose names start with `_` are ignored by all content
pipelines — they get no public URL and never appear in listings, the
`::article-list` directive, the sitemap, or the RSS feed. Use this prefix for
anything under `content/` you don't want published, such as `content/_nav.mdx`.
...
A directory without an `index.mdx` automatically renders a listing of its MDX
files, sorted by `publish-date` descending (undated files sort last). No
...
```mdx
::article-list{dir=blog} ::article-list{dir=blog limit=5}
```
