...
| `rss`         | `'none' \| 'all' \| string[]` | No       | RSS feed content. `'none'` = empty feed (default), `'all'` = all published articles, `string[]` = specific content paths (files or directories relative to `content/`, no `.mdx` extension). The feed is served at `/rss.xml` and is statically prerendered. |
